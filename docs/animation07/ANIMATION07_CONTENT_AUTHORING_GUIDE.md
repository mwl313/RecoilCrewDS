...
## Monster Pack 10 (Quaternius)

- Native Quaternius profiles live in
  `content/enemy-animation-profiles/quaternius/` (45 hero + 15 common) and
  `content/enemy-presentation-profiles/quaternius/`. The animation-content
  generator recurses into subfolders.
- Profiles are generated by `npm run import:monsterpack` from the standalone
  `animation_profiles.json`; every clip is validated against the actual GLB
  by `npm run validate:enemy-animations`.
- Common profiles only map clips present in the common-near GLB and use
  semantic fallbacks for stripped roles. `rootMotion` is always false.
