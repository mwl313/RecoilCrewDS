...
describe('content pack loading (valid Demo pack)', () => {
  it('loads the shipped Demo pack from disk with every category', () => {
    ...
    expect(pack.ids('modes')).toEqual(['mode.demoScoreAttack']);
    expect(pack.ids('objectives')).toEqual(['objective.highScore']);
    expect(pack.ids('tanks')).toEqual(['tank.default']);
    expect(pack.ids('loadouts')).toEqual(['loadout.default']);
    ...
    expect(pack.ids('items')).toEqual([]);
    expect(pack.ids('statusEffects')).toEqual([]);
    expect(pack.ids('spawnDirectors')).toEqual(['spawn.director.demoScoreAttack']);
  ...
  it('shipped values match the current config (spot checks)', () => {
    ...
    expect(mg.damage).toBe(BASE_CONFIG.weapons.mgDamage);
    expect((mg as { rate: number }).rate).toBe(BASE_CONFIG.weapons.mgRate);
    const cannon = pack.getWeapon('weapon.mainCannon');
    expect((cannon as { cooldown: number }).cooldown).toBe(BASE_CONFIG.weapons.cannonCooldown);
    const jackpot = pack.getWeapon('weapon.jackpotShell');
    expect((jackpot as { chargeTime: number }).chargeTime).toBe(BASE_CONFIG.weapons.jackpotChargeTime);
