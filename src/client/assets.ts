...
import { AssetRegistry } from '../shared/assetRegistry';
...
function cyl(rt: number, rb: number, h: number, mat: THREE.Material, seg = 12, x = 0, y = 0, z = 0): THREE.Mesh {
...
function buildTankChassis(): THREE.Object3D {
  ...
  root.add(box(0.35, 0.24, 1.5, dark, 1.28, 0.78, 0));
  // Driver hatch + stabilizer details.
...
function buildTankTurret(): THREE.Object3D {
  const root = new THREE.Group();
...
function buildTankBarrel(): THREE.Object3D {
  const root = new THREE.Group();
  const mat = dark;
  root.add(cyl(0.11, 0.14, 2.8, mat, 10, 0, 0.72, 1.4));
  root.add(box(0.42, 0.34, 0.7, paint(0x1f2c33), 0, 0.62, 0.15));
  root.add(cyl(0.18, 0.2, 0.5, mat, 10, 0, 0.78, 2.75));
  return root;
}

function buildScrapBug(): THREE.Object3D {
  const root = new THREE.Group();
...
function buildRammer(): THREE.Object3D {
  const root = new THREE.Group();
...
function buildGunTower(): THREE.Object3D {
  const root = new THREE.Group();
...
function buildLootTruck(): THREE.Object3D {
  const root = new THREE.Group();
...
function buildPickup(kind: 'normal' | 'heavy' | 'jackpot'): THREE.Object3D {
  const root = new THREE.Group();
...
function buildBarrelProp(): THREE.Object3D {
  const root = new THREE.Group();
...
function buildTireStack(): THREE.Object3D {
  const root = new THREE.Group();
...
function buildContainer(): THREE.Object3D {
  const root = new THREE.Group();
...
function buildRamp(): THREE.Object3D {
  // Unit prism rising along +Z from y=0 to y=1; scaled/rotated per ramp def.
...
function buildFactory(): THREE.Object3D {
  const root = new THREE.Group();
...
export class GameAssets {
  models = new AssetRegistry<THREE.Object3D>();
  vfx = new AssetRegistry<VfxSpec>();
  ui = new AssetRegistry<UiTheme>();
  audio = new AssetRegistry<AudioSpec>();
  gltfLoaders = new Map<string, Promise<THREE.Object3D | null>>();
  ...
  /** Load optional /assets/manifest.json custom-asset overrides. */
  private async loadManifest() {
    try {
      const res = await fetch('/assets/manifest.json', { cache: 'no-cache' });
      if (!res.ok) return;
      const data = (await res.json()) as { assets?: Record<string, unknown>[] };
      if (!Array.isArray(data?.assets)) return;
      for (const a of data.assets) {
        const id = typeof a.id === 'string' ? a.id : '';
        if (!id || !this.models.has(id) && !this.vfx.has(id) && !this.ui.has(id)) continue;
        if (typeof a.file === 'string' && (a.category === 'model' || a.category === undefined)) {
          this.models.registerFile(id, a.file);
        }
        if (a.category === 'vfx') {
          const base = this.vfx.has(id) ? this.vfx.resolve(id) : null;
          if (base) {
            this.vfx.register(id, () => ({
              ...base,
              ...(typeof a.color === 'number' ? { color: a.color } : {}),
              ...(typeof a.size === 'number' ? { size: a.size } : {}),
              ...(typeof a.life === 'number' ? { life: a.life } : {}),
        ...
        if (a.category === 'ui') {
          const base = this.ui.has(id) ? this.ui.resolve(id) : null;
          if (base) {
            this.ui.register(id, () => ({
              ...base,
              ...(typeof a.primary === 'string' ? { primary: a.primary } : {}),
              ...(typeof a.accent === 'string' ? { accent: a.accent } : {}),
              ...(typeof a.panel === 'string' ? { panel: a.panel } : {}),
            }));
          }
        }
      }
    } catch {
      // Manifest is optional; generated fallbacks remain active.
    }
  }

  private registerFallbacks() {
    this.models.register('playerTank.chassis', buildTankChassis);
    this.models.register('playerTank.turret', buildTankTurret);
    this.models.register('playerTank.barrel', buildTankBarrel);
    this.models.register('enemy.scrapBug', buildScrapBug);
    this.models.register('enemy.rammer', buildRammer);
    this.models.register('enemy.gunTower', buildGunTower);
    this.models.register('enemy.lootTruck', buildLootTruck);
    this.models.register('pickup.normalScrap', () => buildPickup('normal'));
    this.models.register('pickup.heavyScrap', () => buildPickup('heavy'));
    this.models.register('pickup.jackpotScrap', () => buildPickup('jackpot'));
    this.models.register('prop.explosiveBarrel', buildBarrelProp);
    this.models.register('prop.barrier', buildBarrier);
    this.models.register('prop.tire', buildTireStack);
    this.models.register('prop.container', buildContainer);
    this.models.register('arena.ramp', buildRamp);
    this.models.register('arena.factory', buildFactory);
    ...
    this.vfx.register('vfx.machineGunMuzzle', () => mkVfx(0xffe08a, 0.16, 0.05, 8, 3));
    this.vfx.register('vfx.cannonMuzzle', () => mkVfx(0xffb347, 0.5, 0.1, 26, 8));
    this.vfx.register('vfx.cannonImpact', () => mkVfx(0xff9d45, 0.5, 0.5, 40, 11, 12));
    this.vfx.register('vfx.enemyDeath', () => mkVfx(0xff5540, 0.5, 0.55, 34, 9, 10));
    this.vfx.register('vfx.scrapPickup', () => mkVfx(0x6fe86f, 0.3, 0.35, 18, 6, 6));
    this.vfx.register('vfx.jackpot', () => mkVfx(0xffe98a, 0.9, 1.0, 80, 14, 8));

    this.ui.register('ui.driverTheme', () => ({
      name: 'DRIVER',
      primary: '#35d7e8',
      accent: '#9ff3ff',
      secondary: '#1c7d8a',
    ...
    this.ui.register('ui.gunnerTheme', () => ({
      name: 'GUNNER',
      primary: '#ffa23b',
      accent: '#ffd08a',
      secondary: '#a85f12',
    ...
    for (const [id, kind] of audioIds) this.audio.register(id, () => mkAudio(kind, id));
  }

  /** Try to load a replacement GLB for a semantic id; fall back to generated. */
  resolveModel(id: string): Promise<THREE.Object3D> {
    const custom = this.models.getFile(id);
    if (custom) {
      if (!this.gltfLoaders.has(id)) {
        this.gltfLoaders.set(
          id,
          import('three/addons/loaders/GLTFLoader.js').then(({ GLTFLoader }) =>
            new Promise<THREE.Object3D | null>((resolve) => {
              new GLTFLoader().load(custom, (gltf) => resolve(gltf.scene), undefined, () => resolve(null));
            }),
          ),
        );
      }
      return this.gltfLoaders.get(id)!.then((loaded) => loaded ?? this.models.resolve(id));
    }
    return Promise.resolve(this.models.resolve(id));
  }
}

export function buildTankRig(assets: GameAssets): TankRig {
  const chassis = assets.models.resolve('playerTank.chassis');
  const turret = assets.models.resolve('playerTank.turret');
  const barrel = assets.models.resolve('playerTank.barrel');
